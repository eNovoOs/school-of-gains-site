...
function storage({saveError,prior}={}){
 const writes=[];const query=async(sql,args)=>{writes.push({sql,args});if(sql.startsWith('SELECT * FROM sog_provider_signals'))return {rows:[{id:'receipt',kind:'appointment',resource_id:'appointment1',contact_id:'contact1',attempts:0}]};if(sql.startsWith('SELECT a.status'))return {rows:prior?[prior]:[]};if(sql.startsWith('SELECT id,email FROM sog_contacts'))return {rows:[{id:'canonical-contact',email:'existing@example.invalid'}]};return {rows:[]};};const db={query,transaction:fn=>fn({query}),saveAppointment:async()=>{if(saveError)throw saveError;},saveOpportunity:async()=>{}};return {db,writes};
}
