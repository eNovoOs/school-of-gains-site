...
test('GHL retries adopt existing opportunity without resetting owner or claiming booking',async()=>{
  ...
  const statements=[];const connection=fakeConnection(async(sql,params)=>{statements.push({sql,params});if(sql.startsWith('SELECT * FROM sog_contacts'))return {rows:[{id:'local',ghl_contact_id:'ghl'}]};if(sql.startsWith('SELECT * FROM sog_sales_cycles'))return {rows:[{id:'cycle'}]};if(sql.startsWith('SELECT id,created_at'))return {rows:[{id:'later-submission'}]};return {rows:[]};});
  try{const result=await syncApplication(connection,{contactId:'local',cycleId:'cycle',contact:{email:'p@example.com'},answers:{},attribution:{firstTouch:{},latestTouch:{}}});assert.equal(result.opportunityId,'existing');assert.equal(requests.length,1);assert.equal(requests[0].options.method,'GET');assert.ok(requests[0].url.includes('status=all'));assert.equal(statements.find(s=>s.sql.includes('SET stage')).params[1],'legacy_review');}finally{global.fetch=oldFetch;for(const key of Object.keys(process.env))if(!(key in oldEnv))delete process.env[key];Object.assign(process.env,oldEnv);}
});
...
test('late retry cannot overwrite fields from the latest application',async()=>{
  ...
  const connection=fakeConnection(async sql=>{if(sql.startsWith('SELECT * FROM sog_contacts'))return {rows:[{id:'local',ghl_contact_id:'ghl'}]};if(sql.startsWith('SELECT * FROM sog_sales_cycles'))return {rows:[{id:'cycle',ghl_opportunity_id:'existing'}]};if(sql.startsWith('SELECT id,created_at'))return {rows:[{id:'new-submission'}]};return {rows:[]};});
...
test('late appointment changes stay on their original closed sales cycle',async()=>{
  process.env.DATABASE_URL='postgres://localhost/not_connected';const pool=db.getPool();const original=pool.connect;const queries=[];
  pool.connect=async()=>fakeConnection(async(sql,params)=>{queries.push({sql,params});if(sql.startsWith('SELECT 1 FROM sog_events'))return {rowCount:0,rows:[]};if(sql.startsWith('SELECT * FROM sog_contacts'))return {rows:[{id:'contact'}]};if(sql.startsWith('SELECT * FROM sog_appointments'))return {rows:[{cycle_id:'original-cycle',updated_at:'2026-09-21T10:00:00Z',attribution:{}}]};if(sql.startsWith('SELECT * FROM sog_sales_cycles'))return {rows:[{id:'original-cycle',status:'lost',stage:'lost'}]};return {rows:[]};});
  try{await db.saveAppointment({appointmentId:'a',contactId:'ghl',eventId:randomUUID(),status:'cancelled',updatedAt:'2026-09-21T12:00:00Z'});const lookup=queries.find(x=>x.sql.startsWith('SELECT * FROM sog_sales_cycles'));assert.deepEqual(lookup.params,['original-cycle']);assert.equal(queries.filter(x=>x.sql.startsWith('UPDATE sog_sales_cycles')).length,0);}finally{pool.connect=original;delete process.env.DATABASE_URL;}
});

test('new provider revision in an unmapped stage requires review instead of retaining a misleading mapped stage',async()=>{
 const oldEnv={...process.env};process.env.DATABASE_URL='postgres://localhost/not_connected';process.env.GHL_STAGE_IDS_JSON=JSON.stringify({booked:'known'});const pool=db.getPool(),original=pool.connect,writes=[];
 pool.connect=async()=>fakeConnection(async(sql,params)=>{if(sql.startsWith('SELECT 1 FROM sog_events'))return {rows:[],rowCount:0};if(sql.startsWith('SELECT * FROM sog_sales_cycles'))return {rows:[{id:'cycle',contact_id:'contact',status:'open',stage:'booked',provider_updated_at:'2026-09-21T11:00:00Z'}]};if(sql.startsWith('UPDATE sog_sales_cycles'))writes.push(params);return {rows:[]};});
 try{await db.saveOpportunity({eventId:randomUUID(),opportunityId:'opp',pipelineStageId:'unmapped',status:'open',updatedAt:'2026-09-21T12:00:00Z'});assert.deepEqual(writes,[['cycle','open','legacy_review','2026-09-21T12:00:00Z']]);}finally{pool.connect=original;for(const key of Object.keys(process.env))if(!(key in oldEnv))delete process.env[key];Object.assign(process.env,oldEnv);}
});
