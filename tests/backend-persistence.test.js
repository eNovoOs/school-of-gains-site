...
test('GHL retries adopt existing opportunity without resetting owner or claiming booking',async()=>{
  const oldFetch=global.fetch;const oldEnv={...process.env};Object.assign(process.env,{GHL_SYNC_ENABLED:'true',GHL_PRIVATE_INTEGRATION_TOKEN:'test',GHL_UNBOOKED_STAGE_ID:'unbooked-test',GHL_DEFAULT_CLOSER_ID:'closer-test',GHL_ATTRIBUTION_FIELDS_JSON:JSON.stringify(Object.fromEntries(['application_id','cycle_id','quiz_version','answers','first_source','first_medium','first_campaign','latest_source','latest_medium','latest_campaign'].map(k=>[k,k])))});
...
test('late retry cannot overwrite fields from the latest application',async()=>{
  const oldFetch=global.fetch;const oldEnv={...process.env};Object.assign(process.env,{GHL_UNBOOKED_STAGE_ID:'unbooked-test',GHL_DEFAULT_CLOSER_ID:'closer-test'});global.fetch=async()=>{throw new Error('unexpected_remote_write')};
