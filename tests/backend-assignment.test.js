...
test('existing open opportunity retains owner and consumes no rotation turn',async()=>{
  ...
  const c={query:async(sql,p)=>{writes.push({sql,p});if(sql.startsWith('SELECT * FROM sog_contacts'))return {rows:[{id:'local',ghl_contact_id:'ghl'}]};if(sql.startsWith('SELECT * FROM sog_sales_cycles'))return {rows:[{id:randomUUID()}]};if(sql.startsWith('SELECT id FROM sog_applications'))return {rows:[{id:'later-submission'}]};return {rows:[]};}};
