...
async function publicRequest(req,res,kind) {
  ...
  await rateLimit(key,['application','booking'].includes(kind) ? 8 : 90);
}
...
function bookingPath(applicationId) {
  if (!process.env.HANDOFF_SECRET || process.env.HANDOFF_SECRET.length < 32) return '/book';
  const value = Buffer.from(JSON.stringify({id:applicationId,expires:Date.now()+3600000})).toString('base64url');
