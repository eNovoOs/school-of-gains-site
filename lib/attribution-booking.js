...
async function contextFor(token) {
  ...
  const {rows:[context]}=await db.query('SELECT a.id,a.contact_id,a.cycle_id,a.attribution,c.ghl_contact_id,o.status AS crm_status FROM sog_applications a JOIN sog_contacts c ON c.id=a.contact_id LEFT JOIN sog_outbox o ON o.event_id=a.id WHERE a.id=$1',[claim.id]);
...
async function reconcile(context,input) {
  ...
  const candidates=(result.events || []).filter(event=>event.calendarId===config().calendarId && Date.parse(event.startTime)===Date.parse(input.startTime));
  if(!candidates.length) return null;
  if(candidates.length>1) throw new InputError('appointment_already_exists',409);
  const event=await verifyAppointment(candidates[0].id,context,input);
  if(event.description!=='School of Gains booking reference '+input.bookingId) throw new InputError('appointment_already_exists',409);
  return event;
}
...
async function book(context,input) {
  ...
  try {
    ...
    if(!created.id)throw new Error('ghl_booking_response_invalid');
    await db.query("UPDATE sog_booking_intents SET appointment_id=$2,updated_at=now() WHERE id=$1",[input.bookingId,created.id]);
    return complete(context,input,await verifyAppointment(created.id,context,input));
  } catch(error) {
