...
async function contextFor(token) {
  ...
  const {rows:[context]}=await db.query('SELECT a.id,a.contact_id,a.cycle_id,a.attribution,c.email,c.ghl_contact_id,o.status AS crm_status FROM sog_applications a JOIN sog_contacts c ON c.id=a.contact_id LEFT JOIN sog_outbox o ON o.event_id=a.id WHERE a.id=$1',[claim.id]);
