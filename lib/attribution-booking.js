...
async function complete(context,input,event) {
  const canonical=await db.transaction(async c=>{
    await db.saveAppointmentInTransaction(c,{eventId:input.bookingId,locationId:LOCATION,appointmentId:event.id,contactId:context.ghl_contact_id,calendarId:event.calendarId,status:event.appointmentStatus,startsAt:event.startTime,updatedAt:event.dateUpdated,bookingSetterId:null,applicationId:context.id,attribution:context.attribution});
    // The helper retains appointment/contact/cycle locks. Derive intent state
    // from the winning canonical revision, including a newer cancellation.
    const {rows:[appointment]}=await c.query('SELECT status,starts_at FROM sog_appointments WHERE id=$1',[event.id]);
    if(!appointment)throw new Error('booking_canonical_missing');
    const state=['cancelled','invalid'].includes(appointment.status)?'cancelled':'confirmed';
    await c.query('UPDATE sog_booking_intents SET state=$3,appointment_id=$2,updated_at=now() WHERE id=$1',[input.bookingId,event.id,state]);
    return appointment;
  });
  if(!['new','confirmed'].includes(canonical.status))throw new InputError('appointment_changed',409);
  return {ok:true,booked:true,appointmentId:event.id,startTime:new Date(canonical.starts_at).toISOString(),timezone:input.timezone};
}
async function book(context,input) {
  ...
  const result=await db.transaction(async c=>{
    ...
    await c.query("INSERT INTO sog_booking_intents(id,application_id,cycle_id,start_time,timezone,state,calendar_id) VALUES($1,$2,$3,$4,$5,'pending',$6)",[input.bookingId,context.id,context.cycle_id,input.startTime,input.timezone,calendarId]);
    return {created:true};
  });
  if(!result.created) {
  ...
  try {
    ...
    if(error instanceof InputError && error.message==='appointment_changed')throw error;
    ...
    await db.query('UPDATE sog_booking_intents SET state=$2,updated_at=now() WHERE id=$1 AND state IN (\'pending\',\'uncertain\')',[input.bookingId,definitive?'failed':'uncertain']);
    if(definitive)throw new InputError(error.statusCode===409?'slot_unavailable':'booking_rejected',error.statusCode===409?409:503);
    return {ok:true,booked:false,pending:true,reason:'booking_verification_pending'};
  }
}
module.exports={claimFor,contextFor,config,requireBooking,range,slots,bookingInput,appointmentPayload,verifyAppointment,reconcile,complete,book};
