...
const leadProvider=require('./lead-ghl');
async function processOne(deps={}) {
 const storage=deps.db || db,application=deps.syncApplication || provider.syncApplication,appointment=deps.syncAppointment || provider.syncAppointment,lead=deps.syncLead || leadProvider.syncLead;
 return storage.transaction(async c=>{
  ...
  try {
   ...
   else if(job.type==='lead')await lead(c,job.payload);
   else if(job.type==='appointment')await appointment(c,job.payload);
   else throw new Error('ghl_unsupported_job');
   if(job.payload.cycleId && ['application','appointment'].includes(job.type))await require('./sales-tasks').reconcileCycle(c,job.payload.cycleId);
