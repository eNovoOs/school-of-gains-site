...
// Carry allowlisted evidence. Only the user-confirmed meetup route has a fallback.
function evidence(value) {
  const result = {};
  if (typeof value.link_id === 'string' && /^sog_link_(00[1-9]|01[0-4])$/.test(value.link_id)) result.link_id = value.link_id;
  if (value.route_map_version === 'v1' && typeof value.legacy_route === 'string' && /^legacy_(00[1-9]|0[1-3][0-9]|04[0-6])$/.test(value.legacy_route)) {
    result.legacy_route = value.legacy_route;
...
function touch(value, now = new Date()) {
  ...
  // Route inference must never manufacture explicit UTMs or override any supplied
  // campaign/click evidence. Other historical suffixes remain unclassified.
  const suppliedTracking=['utm_source','utm_medium','utm_campaign','utm_content','utm_term','utm_id','gclid','fbclid','gbraid','wbraid','ttclid'].some(key=>result[key]);
  if(!suppliedTracking && result.legacy_route==='legacy_001' && result.route_map_version==='v1') {
    result.source='meetup';result.medium='offline';result.confidence='legacy_path_inferred';
  }
  result.model = 'sog-v1';
  return result;
}
...
function nextStage(current, appointmentStatus) {
  if (['won','lost'].includes(current.status) || current.stage==='legacy_review') return current.stage;
  if (['call_held','follow_up'].includes(current.stage) && ['new','confirmed','cancelled','invalid','noshow'].includes(appointmentStatus)) return current.stage;
