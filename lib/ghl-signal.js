...
async function normalize(input,read=provider.request) {
 ...
 if(!opportunity || opportunity.id!==input.resourceId || opportunity.locationId!==LOCATION || ![CLOSERS,SETTERS].includes(opportunity.pipelineId) || (opportunity.pipelineId===SETTERS&&!id(opportunity.contactId)) || !id(opportunity.pipelineStageId) || (input.contactId && opportunity.contactId!==input.contactId))throw new InputError('provider_identity_mismatch',422);
 if(!['open','won','lost',...(opportunity.pipelineId===SETTERS?['abandoned']:[])].includes(opportunity.status))throw new InputError('provider_status_unsupported',422);
 const payload={type:'opportunity_updated',locationId:LOCATION,pipelineId:opportunity.pipelineId,...(opportunity.pipelineId===SETTERS?{contactId:opportunity.contactId}:{}),opportunityId:opportunity.id,pipelineStageId:opportunity.pipelineStageId,status:opportunity.status,updatedAt:revision(opportunity.updatedAt || opportunity.dateUpdated)};
 return {eventId:eventId(payload),...payload,observedVia:'provider_readback'};
}
// Appointment signals may arrive before a contact-created signal. Bind only
// server-read identity; do not manufacture quiz submissions or prospect captures.
async function ensureAppointmentContact(payload,storage=db,read=provider.request,env=process.env) {
 const {rows:[linked]}=await storage.query('SELECT id,email FROM sog_contacts WHERE ghl_contact_id=$1',[payload.contactId]);
 if(linked){require('./intake-pilot').assertPilotContact(linked.email,env);return linked;}
 const {contact}=await read('/contacts/'+encodeURIComponent(payload.contactId));
 if(!contact || contact.id!==payload.contactId || contact.locationId!==LOCATION)throw new InputError('provider_identity_mismatch',422);
 const email=String(contact.email || '').trim().toLowerCase();
 if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))throw new InputError('provider_email_required',422);
 require('./intake-pilot').assertPilotContact(email,env);
 const attribution=require('./lead-capture-signal').remoteAttribution(contact,env);
 return storage.transaction(async c=>{
  await c.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))',[email]);
  const {rows:[concurrent]}=await c.query('SELECT * FROM sog_contacts WHERE ghl_contact_id=$1 FOR UPDATE',[contact.id]);
  if(concurrent){
   if(concurrent.email!==email)throw new InputError('provider_contact_identity_conflict',422);
   return concurrent;
  }
  await c.query('INSERT INTO sog_contacts(id,email,first_touch,latest_touch) VALUES($1,$2,$3,$4) ON CONFLICT(email) DO NOTHING',[randomUUID(),email,attribution.firstTouch,attribution.latestTouch]);
  const {rows:[canonical]}=await c.query('SELECT * FROM sog_contacts WHERE email=$1 FOR UPDATE',[email]);
  if(!canonical || (canonical.ghl_contact_id && canonical.ghl_contact_id!==contact.id))throw new InputError('provider_contact_identity_conflict',422);
  // Repeated native readbacks never overwrite an established first/latest touch.
  // Existing captures and applications retain their original consent evidence.
  try{await c.query('UPDATE sog_contacts SET ghl_contact_id=$2 WHERE id=$1',[canonical.id,contact.id]);}
  catch(error){if(error.code==='23505')throw new InputError('provider_contact_identity_conflict',422);throw error;}
  return {...canonical,ghl_contact_id:contact.id};
 });
}
// Setter stage signals are wakeups, never proof of a booking. No cycle, contact,
// appointment or synthetic application is created here. Keep the receipt lease
// locked while resetting only the precise manual-handoff delivery failure.
async function reconcileSetterHandoff(payload,job,storage=db,read=provider.request,env=process.env){
 if(payload.pipelineId!==SETTERS||!id(payload.contactId))throw new InputError('provider_identity_mismatch',422);
 const {contact}=await read('/contacts/'+encodeURIComponent(payload.contactId));
 if(!contact||contact.id!==payload.contactId||contact.locationId!==LOCATION)throw new InputError('provider_identity_mismatch',422);
 const email=String(contact.email||'').trim().toLowerCase();
 if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))throw new InputError('provider_email_required',422);
 require('./intake-pilot').assertPilotContact(email,env);
 if(payload.status!=='open'||payload.pipelineStageId!==SETTER_BOOKED)return {ignored:true};
 return storage.transaction(async c=>{
  const {rows:[claim]}=await c.query('SELECT id FROM sog_provider_signals WHERE id=$1 AND claim_token=$2 AND lease_until>clock_timestamp() FOR UPDATE',[job.id,job.claimToken]);
  if(!claim)throw new InputError('signal_lease_lost',409);
  const {rows:[linked]}=await c.query('SELECT id,email FROM sog_contacts WHERE ghl_contact_id=$1',[payload.contactId]);
  if(!linked)throw new InputError('setter_contact_not_linked_retry',409);
  if(linked.email!==email)throw new InputError('provider_contact_identity_conflict',422);
  require('./intake-pilot').assertPilotContact(linked.email,env);
  const {rows:booked}=await c.query("SELECT sc.id FROM sog_sales_cycles sc WHERE sc.contact_id=$1 AND sc.status='open' AND EXISTS (SELECT 1 FROM sog_appointments a WHERE a.cycle_id=sc.id AND a.contact_id=sc.contact_id AND a.status IN ('new','confirmed'))",[linked.id]);
  if(!booked.length)throw new InputError('setter_booking_not_linked_retry',409);
  const result=await c.query("UPDATE sog_outbox o SET status='pending',attempts=0,last_error=NULL,available_at=now() WHERE o.type='appointment' AND o.status IN ('pending','failed') AND o.last_error='ghl_manual_setter_transfer_pending' AND EXISTS (SELECT 1 FROM sog_sales_cycles sc JOIN sog_appointments a ON a.cycle_id=sc.id AND a.contact_id=sc.contact_id WHERE sc.contact_id=$1 AND sc.status='open' AND sc.id::text=o.payload->>'cycleId' AND a.id=o.payload->>'appointmentId' AND a.status IN ('new','confirmed'))",[linked.id]);
...
async function processOne(deps={}) {
 ...
 try {
   ...
   if(job.kind==='appointment') {
    ...
    await ensureAppointmentContact(payload,storage,read,deps.env || process.env);
    await storage.saveAppointment(payload);
   }else if(payload.pipelineId===SETTERS){
    await reconcileSetterHandoff(payload,job,storage,read,deps.env || process.env);
   }else {
   ...
   const delivered=await storage.query("UPDATE sog_provider_signals SET status='delivered',normalized_event_id=$2,last_error=NULL,processed_at=now(),lease_until=NULL,claim_token=NULL WHERE id=$1 AND claim_token=$3 AND lease_until>clock_timestamp()",[job.id,payload.eventId,job.claimToken]);
   ...
   await storage.query("UPDATE sog_provider_signals SET status=CASE WHEN $3::boolean OR attempts>=10 THEN 'failed' ELSE 'pending' END,last_error=$2,available_at=now()+make_interval(secs=>LEAST(3600,30*power(2,LEAST(attempts,10)-1))::int),lease_until=NULL,claim_token=NULL WHERE id=$1 AND claim_token=$4 AND lease_until>clock_timestamp()",[job.id,code,terminal,job.claimToken]);
   return {processed:0,retryQueued:!terminal && job.attempts<10,needsReview:terminal || job.attempts>=10};
  }
}
module.exports={signal,revision,eventId,normalize,ensureAppointmentContact,reconcileSetterHandoff,enqueue,processOne};
