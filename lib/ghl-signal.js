...
async function processOne(deps={}) {
 ...
 try {
   ...
   if(job.kind==='appointment') {
    ...
    const stage=Object.entries(mapping).find(([,value])=>value===payload.pipelineStageId)?.[0];
   ...
   await storage.query("UPDATE sog_provider_signals SET status='delivered',normalized_event_id=$2,last_error=NULL,processed_at=now(),lease_until=NULL,claim_token=NULL WHERE id=$1 AND claim_token=$3",[job.id,payload.eventId,job.claimToken]);
   return {processed:1};
  }catch(error){
