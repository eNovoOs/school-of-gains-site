...
async function linkBookedOpportunity(c,cycle,stages,deps={}) {
  ...
  if(result.opportunities.some(o=>o.contactId!==contactId || o.pipelineId!==CLOSERS || (o.locationId && o.locationId!==LOCATION)))throw new Error('ghl_opportunity_review_required');
  const active=result.opportunities.filter(o=>o.status==='open');
  if(active.length>1)throw new Error('ghl_multiple_active_opportunities');
  let opportunity=active[0];
  if(!opportunity && result.opportunities.some(o=>o.status==='won')) {
    await c.query("UPDATE sog_sales_cycles SET status='review',stage='customer_review' WHERE id=$1",[cycle.id]);
    return null;
  }
  if(!opportunity) {
