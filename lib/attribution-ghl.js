...
async function linkBookedOpportunity(c,cycle,stages,deps={}) {
  ...
  const result=await request('/opportunities/search?'+search);
  if(!Array.isArray(result.opportunities) || result.meta?.nextPage || result.meta?.nextPageUrl || result.opportunities.length>=100)throw new Error('ghl_opportunity_review_required');
  ...
  if(!opportunity) {
    const setterPipeline='PSq0fv77HbtMg9bdKC2p';
    // A setter books first and moves their existing deal manually. Wait for
    // that transfer instead of creating a competing closer deal in the gap.
    if(result.opportunities.some(o=>o.pipelineId===setterPipeline && o.status==='open'))throw new Error('ghl_manual_setter_transfer_pending');
    // A just-moved deal may lag behind search indexing. Read our known deal
    // identity directly before authorizing any new opportunity creation.
    const {rows:[leadCycle]}=await c.query('SELECT ghl_opportunity_id FROM sog_lead_cycles WHERE contact_id=$1',[local.id]);
    if(leadCycle?.ghl_opportunity_id) {
      const {opportunity:known}=await request('/opportunities/'+encodeURIComponent(leadCycle.ghl_opportunity_id));
      if(!known || known.id!==leadCycle.ghl_opportunity_id || known.contactId!==contactId || (known.locationId && known.locationId!==LOCATION))throw new Error('ghl_opportunity_review_required');
      if(known.status==='open' && known.pipelineId===setterPipeline)throw new Error('ghl_manual_setter_transfer_pending');
      if(known.status==='open' && known.pipelineId===CLOSERS)opportunity=known;
      else if(!['lost','abandoned'].includes(known.status))throw new Error('ghl_manual_setter_transfer_review_required');
    }
  }
  if(!opportunity) {
    ...
    if(cycle.stage==='booked') {
      ...
      targetKey=routing.bookingStage(stages,null,appointment);
    }
