...
async function request(path,method='GET',data) {
  ...
  const response = await fetch('https://services.leadconnectorhq.com'+path,{method,headers:{Authorization:`Bearer ${token}`,Version:'2021-07-28','Content-Type':'application/json'},...(data?{body:JSON.stringify(data)}:{}),signal:AbortSignal.timeout(12000)});
...
async function syncApplication(c,input) {
  if (!process.env.GHL_UNBOOKED_STAGE_ID || !process.env.GHL_DEFAULT_CLOSER_ID) throw new InputError('ghl_routing_not_configured',503);
  ...
  if(!opportunity) {
    const created=await request('/opportunities/','POST',{locationId:LOCATION,pipelineId:CLOSERS,pipelineStageId:process.env.GHL_UNBOOKED_STAGE_ID,contactId,name:'Apprentice application',status:'open',assignedTo:process.env.GHL_DEFAULT_CLOSER_ID});
    opportunity=created.opportunity;
  }
  if(!opportunity?.id) throw new Error('ghl_opportunity_response_invalid');
  await c.query('UPDATE sog_sales_cycles SET ghl_opportunity_id=$2 WHERE id=$1',[cycle.id,opportunity.id]);
