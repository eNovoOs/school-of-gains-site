...
const ATTR_KEYS = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content',
  'fbclid', 'gclid', 'ttclid', 'landing_page', 'referrer', 'captured_at'];

// Whitelist + trim the attribution object sent by the browser.
function cleanAttribution(a) {
  const out = {};
  if (!a || typeof a !== 'object') return out;
  for (const k of ATTR_KEYS) if (a[k]) out[k] = String(a[k]).slice(0, 500);
  return out;
}

function leadPayload(req, email, stage, extra, attribution) {
  const h = req.headers || {};
...
module.exports = { sendToGHL, leadPayload, cleanAttribution };
