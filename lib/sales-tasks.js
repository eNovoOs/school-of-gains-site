...
function dueHours(){
 const value=Number(process.env.GHL_TASK_DUE_HOURS);
 if(!Number.isFinite(value)||value<=0||value>168)throw new InputError('task_due_policy_missing',503);
 return value;
}
...
async function reconcileCycle(c,cycleId){
 ...
 const {rows:[cycle]}=await c.query('SELECT sc.*,ct.ghl_contact_id FROM sog_sales_cycles sc JOIN sog_contacts ct ON ct.id=sc.contact_id WHERE sc.id=$1 FOR UPDATE OF sc',[cycleId]);
 ...
 const {rows:[created]}=await c.query("INSERT INTO sog_sales_tasks(id,cycle_id,episode,ghl_contact_id,ghl_opportunity_id,due_at) SELECT $1,$2,COALESCE(MAX(episode),0)+1,$3,$4,now()+make_interval(secs=>$5) FROM sog_sales_tasks WHERE cycle_id=$2 RETURNING id",[randomUUID(),cycleId,cycle.ghl_contact_id,cycle.ghl_opportunity_id,Math.round(dueHours()*3600)]);
...
module.exports={reconcileCycle,processOne,findTask,verifyTask,verifyOpportunity,marker,isUnbooked,dueHours};
