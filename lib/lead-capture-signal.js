...
function remoteAttribution(contact,env=process.env){
 ...
 const fields=new Map((contact.customFields || []).map(field=>[field.id,field.value ?? field.fieldValue]));
 ...
 const decode=prefix=>{
  ...
  // Preserve provider-saved attribution classification, including inferred source.
  for(const key of ['source','medium']){const value=raw[key] || read(prefix+'_'+key);result[key]=typeof value==='string' && value.trim()?value.trim().slice(0,200):raw['utm_'+key]?result[key]:'unknown';}
  if(!result.utm_campaign && typeof read(prefix+'_campaign')==='string')result.utm_campaign=read(prefix+'_campaign').slice(0,200);
  ...
  result.confidence=['explicit','inferred','unknown'].includes(raw.confidence)?raw.confidence:'unknown';return result;
 };
