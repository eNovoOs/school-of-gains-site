...
async function saveAppointment(input) { return transaction(c=>saveAppointmentInTransaction(c,input)); }
async function saveAppointmentInTransaction(c,input) {
  ...
  // A verified calendar booking qualifies an existing canonical contact even
  // without a quiz. The contact row lock serializes concurrent bookings/intake.
  // Never attach an old closed cycle's appointment to a newly opened cycle.
  if(!cycle && !old?.cycle_id && !input.applicationId && ['new','confirmed','showed'].includes(input.status)) {
    const won=await c.query("SELECT 1 FROM sog_sales_cycles WHERE contact_id=$1 AND status='won' LIMIT 1",[contact.id]);
    const created=await c.query('INSERT INTO sog_sales_cycles(id,contact_id,status,stage) VALUES($1,$2,$3,$4) RETURNING *',[randomUUID(),contact.id,won.rowCount?'review':'open',won.rowCount?'customer_review':input.status==='showed'?'call_held':'booked']);
    cycle=created.rows[0];
  }
  const snapshot = old?.attribution || input.attribution || {firstTouch:contact.first_touch,latestTouch:contact.latest_touch};
  await c.query('INSERT INTO sog_appointments(id,contact_id,cycle_id,calendar_id,status,starts_at,updated_at,attribution,booking_setter_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT(id) DO UPDATE SET cycle_id=COALESCE(sog_appointments.cycle_id,EXCLUDED.cycle_id),status=EXCLUDED.status,starts_at=EXCLUDED.starts_at,updated_at=EXCLUDED.updated_at,booking_setter_id=COALESCE(sog_appointments.booking_setter_id,EXCLUDED.booking_setter_id)',[input.appointmentId,contact.id,cycle?.id || null,input.calendarId,input.status,input.startsAt,input.updatedAt,snapshot,input.bookingSetterId]);
...
async function saveOpportunity(input) { return transaction(async c=>{
  ...
  const mapped=require('./pipeline-routing').lifecycleStage(stages,input.pipelineStageId);
  ...
  await require('./sales-tasks').reconcileCycle(c,cycle.id);
  return {duplicate:false};
}); }
...
module.exports = {getPool,query,transaction,saveEvent,saveApplication,saveAppointment,saveAppointmentInTransaction,saveOpportunity,rateLimit};
