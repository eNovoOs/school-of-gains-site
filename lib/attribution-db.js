...
async function saveAppointment(input) { return transaction(async c => {
  ...
  const {rows:[cycle]} = await c.query("SELECT * FROM sog_sales_cycles WHERE contact_id=$1 AND offer_id='apprentice' AND status IN ('open','review') ORDER BY created_at DESC LIMIT 1 FOR UPDATE",[contact.id]);
...
async function saveOpportunity(input) { return transaction(async c=>{
  ...
  const stage=input.status==='open'?(mapped || cycle.stage):input.status;
