...
- `GET` or `POST /api/webhooks/process`: server bearer `CRON_SECRET`, processes a bounded serial batch of durable outbox jobs with row locking (default 5 jobs and a 15-second soft deadline). Configure a protected schedule/worker after live validation; no cron has been enabled. Native Vercel cron runs only on production deployments; invoke preview workers manually during controlled validation. Retries exponential backoff up to ten attempts, then a failed queue for review. A scheduler invocation alone does not retry failed records indefinitely.
